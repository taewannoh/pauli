...
#added by Taewan
NODE_test_s                   = BCTYPE_test_s
NODE_test_n                   = BCTYPE_test_n
#addition ended
...
class BoundaryConditionList(list):
    ...
    def __init_boundary_conditions(self):
        ...
        bctypes = (
            ...
            #Added by Taewan starts
	    ( lambda ntype: ntype == NODE_test_s,
              self.append_s_terminal_spectral,
              self.append_s_terminal_kinetic ),
            ( lambda ntype: ntype == NODE_test_n,
              self.append_n_terminal_spectral,
              self.append_n_terminal_kinetic )
	   #Addtion by Taewan ends
            )
    ...
    ### test s terminals added by Taewan
    def append_s_terminal_kinetic(self):
        """Boundary conditions at an S terminal."""
        if self.subgap:
            self.append_terminal_bcs(BCTYPE_test_s, (1,3,))
        else:
            self.append_terminal_bcs(BCTYPE_test_n, (1,2))

    def append_s_terminal_spectral(self):
        """Boundary conditions at an S terminal."""
        self.append_terminal_bcs(BCTYPE_test_s, (1,2,3,4))
       
    ### test n terminals added by Taewan
    def append_n_terminal_kinetic(self):
        """Boundary conditions at an N terminal."""
        self.append_terminal_bcs(BCTYPE_test_n, (1,2))
    def append_n_terminal_spectral(self):
        """Boundary conditions at an N terminal."""
        self.append_terminal_bcs(BCTYPE_test_n, (1,2,3,4))  
